...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func update(currentTime: CFTimeInterval) {
        if !gameOver {
            ...
            //this is just testing if it is an emulator or device
    ...
    func playerCollidedWithNode(node: SKNode) {
        if node.name == "vortex" {
            ...
            // next level?
            //add things that make the new levels different – perhaps a teleport that moves the player from one point in the level to another? Add a new letter type in loadLevel(), add another collision type to our enum, then see what you can do. 
        }
    }
}
